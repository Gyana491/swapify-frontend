...
const Listings = () => {
    ...
    const fetchListings = async (maxDistance) => {
        try {
            setLoading(true);
            ...
            if (latitude && longitude && !isNaN(parseFloat(latitude)) && !isNaN(parseFloat(longitude))) {
                const response = await axios.get(
                    `${process.env.NEXT_PUBLIC_HOST}/api/listings/nearby`,
                    {
                        params: {
                ...
                if (response.data.message) {
                    setError(response.data.listings.length === 0 ? response.data.message : null);
                }
            } else {
    ...
    const handleDistanceChange = async (distance) => {
        ...
        await fetchListings(distance);
    };

    useEffect(() => {
        fetchListings(selectedDistance);
    }, []); // Only fetch on mount
        ...
        <>
            ...
            <div className="max-w-screen-xl mx-auto px-4 py-4">
                <div className="relative">
                    <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none dark:from-gray-900"></div>
                    <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none dark:from-gray-900"></div>
                    <div className="overflow-x-auto scrollbar-hide">
                        <div className="flex gap-2 mb-4 pb-2 min-w-min">
                            {DISTANCE_OPTIONS.map((option) => (
                                <button
                                    key={option.value}
                                    onClick={() => handleDistanceChange(option.value)}
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors duration-200 whitespace-nowrap flex-shrink-0 ${
